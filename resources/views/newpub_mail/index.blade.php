					...
					<div class="row">
						<div class="col-sm-3 col-xs-12 mail_list_column">
							<ul class="list-group mail-list">
								<a href="{{ route('newpub.mail.create') }}" class="btn btn-success center-block"
								...
								<table class="table table-hover table-bordered" id="mail_list">
									...
									<tbody>
										...
										<tr>
											...
											<td>
												<a href="{{ route('newpub.mail.show', $mail->mail_id) }}">{{ $mail->title }}</a>
											...
											<td>
												<a href="javascript:;" onclick="deleteComfirm('{{ route('newpub.mail.destroy', [$mail->mail_id, $active == 'inbox' ? 'i' : 's']) }}')" class="btn btn-danger btn-sm"> <i class="fa fa-trash"></i>
												</a>
												<a href="{{ route('newpub.mail.show', $mail->mail_id) }}" class="btn btn-success btn-sm"> <i class="fa fa-eye"></i>
												</a>
											</td>
										</tr>
										@endforeach
...
<script>
	...
	$('#delete_checked').click(function () {
		...
		$.ajax({
			url: '{{ route('newpub.mail.destroy.list') }}',
			method: 'POST',
	...
	$('#save_check').click(function () {
		...
		$.ajax({
			url: '{{ route('newpub.mail.save.list') }}',
			method: 'POST',
